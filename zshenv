export LANG=en_US.UTF-8
export LC_ALL=en_US.UTF-8
export HISTCONTROL=erasedups
export LESS="-CQaixr4"
export DOTFILES="$HOME/Dotfiles"
export VIMCONFIG="$HOME/.vim"
export VIMDATA="$HOME/.vim"
export CURL_BIN=/usr/local/opt/curl/bin
export OPENSSL_BIN=/usr/local/opt/openssl/bin
# export CC=$(which clang)
# export CXX=$(which clang++)
export SDKMAN_DIR="$HOME/.sdkman"
export NVM_HOME="$HOME/.nvm"
export GOROOT="$(/usr/local/bin/brew --prefix go)/libexec"
export GOPATH="$HOME/Workspace/go"
export ANDROID_SDK_ROOT=/usr/local/share/android-sdk
export FLUTTER_HOME=/usr/local/share/flutter
export CARGO_HOME="$HOME/.cargo"
export PATH="$HOME/.local/bin:$HOME/local/bin:$HOME/bin:$OPENSSL_BIN:$CURL_BIN:/usr/local/bin:$VIMCONFIG/pack/bundle/start/fzf/bin:$GOROOT/bin:$GOPATH/bin:$ANDROID_SDK_ROOT/build-tools:$ANDROID_SDK_ROOT/emulator:$ANDROID_SDK_ROOT/tools:$ANDROID_SDK_ROOT/tools/bin:$ANDROID_SDK_ROOT/platform-tools:$FLUTTER_HOME/bin:$CARGO_HOME/bin:$PATH"
